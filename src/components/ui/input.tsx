  ...
  `,
  {
    variants: {
      variant: {
        ...
        md: "h-10 px-4 text-sm rounded-md file:pe-4 file:me-4",
        sm: "h-8 px-2 text-xs rounded-md file:pe-2 file:me-2",
      },
    },
    defaultVariants: {
      variant: "md",
...
const inputAddonVariants = cva(
  "flex items-center shrink-0 justify-center bg-muted border border-input shadow-xs shadow-[rgba(0,0,0,0.05)] text-muted-foreground [&_svg]:text-muted-foreground/60",
  {
    variants: {
      variant: {
        lg: "rounded-md h-10 min-w-10 px-4 text-sm [&_svg:not([class*=size-])]:size-4.5",
        md: "rounded-md h-10 min-w-10 px-4 text-sm [&_svg:not([class*=size-])]:size-4.5",
        sm: "rounded-md h-8 min-w-8 text-xs px-2 [&_svg:not([class*=size-])]:size-3.5",
      },
      mode: {
        default: "",
 ...
 *
 * Provides consistent styling and behavior for form inputs with support
 * for different sizes and validation states.
 *
 * @component
 */
function Input({
